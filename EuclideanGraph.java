...
public class EuclideanGraph {
    ...
    *******************************************************************/
    public EuclideanGraph(In in) {
        ...
        points = new Point[V]; //new point of vertices
        for (int i = 0; i < V; i++) {
            int v = Integer.parseInt(in.readString());
            int x = Integer.parseInt(in.readString());
            int y = Integer.parseInt(in.readString());
            if (v < 0 || v >= V) throw new RuntimeException("Illegal vertex number"); //if number of vertices is incorrect
            points[v] = new Point(x, y); //points of vertices make new point of x and y 
        }

        // read in and insert edges
        adj = new Node[V]; //new point of edges
        for (int i = 0; i < E; i++) {
            int v = Integer.parseInt(in.readString());
            int w = Integer.parseInt(in.readString());
            if (v < 0 || v >= V) throw new RuntimeException("Illegal vertex number"); // if wrong number of edges
            if (w < 0 || w >= V) throw new RuntimeException("Illegal vertex number"); //if wrong number of edges
            adj[v] = new Node(w, adj[v]); //adj[v] = new node object of w, adj[v]
            adj[w] = new Node(v, adj[w]);//adj[w] = new node object of v, adj[w]
        }
    }
